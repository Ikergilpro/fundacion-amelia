...
export async function generateMetadata({ params }: ProgramPageProps) {
  ...
  if (!program) {
    return pageMetadata({
      title: "Programa",
      description: "Programa de Fundación AMELIA A.C.",
      path: "/programas",
    });
  }
  return pageMetadata({
    ...
    <>
      <section className="bg-ivory py-12 lg:py-16">
        <Container>
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
            ...
            <Button href="/contacto?motivo=orientacion">Solicitar orientación</Button>
          </div>
        </Container>
      </section>
      ...
      <section className="py-16">
        <Container className="grid gap-12 lg:grid-cols-[1.4fr_0.8fr]">
          <div>
            <h2 className="font-serif text-2xl text-navy">Actividades principales</h2>
          ...
          <aside className="space-y-6">
            <div className="rounded-2xl border border-navy/8 bg-white p-6">
              <h2 className="font-serif text-xl text-navy">Dirigido a</h2>
            ...
            {related.length > 0 ? (
              <div className="rounded-2xl border border-navy/8 bg-white p-6">
                <h2 className="font-serif text-xl text-navy">Programas relacionados</h2>
                <ul className="mt-4 space-y-3">
                  {related.map((item) => (
                    <li key={item.slug}>
                      <Button href={`/programas/${item.slug}`} variant="outline" size="sm">
                        {item.name}
                      </Button>
                    </li>
                  ))}
